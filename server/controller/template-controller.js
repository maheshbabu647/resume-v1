...
const createTemplate = async (req, res, next) => {
  try {
    ...
    const { templateName, layoutSlots, templateComponents, templateFieldDefinition, tags, isAtsRecommended, presets } = req.body;
    let parsedLayoutSlots, parsedTemplateComponents, parsedTemplateFieldDefinition, parsedTags, parsedIsAtsRecommended, parsedPresets;
    ...
    // --- NEW: Parse isAtsRecommended ---
    if (isAtsRecommended !== undefined) {
      // Handles "true" or "false" string from form data
      parsedIsAtsRecommended = isAtsRecommended === 'true';
    }

    // --- NEW: Parse and validate presets ---
    if (presets) {
      try {
        parsedPresets = JSON.parse(presets);
        if (!Array.isArray(parsedPresets)) throw new Error('Presets must be a valid JSON array.');
      } catch (error) {
        logger.warn(`[Template][Create][ValidationFail] Invalid presets: ${error.message}`);
        const err = new Error(error.message || 'Invalid format for presets.');
        err.status = 400;
        return next(err);
      }
    }
    ...
    const newTemplate = {
      ...
      templateImage: templateImageUrl,
      templateImageId: templatePublicId,
      isAtsRecommended: parsedIsAtsRecommended || false, // Default to false if not provided
      presets: parsedPresets || [], // Default to an empty array
      layoutSlots: parsedLayoutSlots,
      templateComponents: parsedTemplateComponents,
      templateFieldDefinition: parsedTemplateFieldDefinition || [],
      tags: parsedTags || {},
...
const updateTemplate = async (req, res, next) => {
  try {
    ...
    // --- NEW: Parse isAtsRecommended if provided ---
    if (isAtsRecommended !== undefined) {
      parsedIsAtsRecommended = isAtsRecommended === 'true';
    }

    // --- NEW: Parse and validate presets if provided ---
    if (presets !== undefined) {
      try {
        parsedPresets = JSON.parse(presets);
        if (!Array.isArray(parsedPresets)) throw new Error('Presets must be a valid JSON array.');
      } catch (error) {
        logger.warn(`[Template][Update][ValidationFail] Invalid presets: ${error.message}`);
        const err = new Error(error.message || 'Invalid format for presets.');
        err.status = 400;
        return next(err);
      }
    ...
    // Parse and validate other fields only if they are provided in the request
    if (layoutSlots !== undefined) {
      try {
        parsedLayoutSlots = JSON.parse(layoutSlots);
        if (!Array.isArray(parsedLayoutSlots)) throw new Error('layoutSlots must be a valid JSON array.');
    ...
    // Construct the update object, falling back to existing data if a field is not provided
    const templateNewData = {
      templateName: templateName || template.templateName,
      templateImage: updatedImageUrl,
      templateImageId: updatedImageId,
      // Use nullish coalescing to allow setting `isAtsRecommended` to false
      isAtsRecommended: parsedIsAtsRecommended ?? template.isAtsRecommended,
      presets: parsedPresets || template.presets,
      layoutSlots: parsedLayoutSlots || template.layoutSlots,
      templateComponents: parsedTemplateComponents || template.templateComponents,
      templateFieldDefinition: parsedTemplateFieldDefinition || template.templateFieldDefinition,
      tags: parsedTags || template.tags,
