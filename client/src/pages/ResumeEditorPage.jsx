...
//                           <SelectContent>
//                             {/* FIX: Replaced invalid SelectItem with a simple text message */}
...
const ResumeEditorPage = () => {
  ...
  const location = useLocation();
  const resumePreviewRef = useRef();
  const nameInputRef = useRef(null); // Ref for resume name input
  ...
  const [currentTemplateForEditor, setCurrentTemplateForEditor] = useState(null);
  const [editableResumeName, setEditableResumeName] = useState('');
  const [isEditingName, setIsEditingName] = useState(false); // State for name edit mode
  const [isDownloadingPdf, setIsDownloadingPdf] = useState(false);
  const [showFeedbackDialog, setShowFeedbackDialog] = useState(false);
  const [feedbackDetailsForDialog, setFeedbackDetailsForDialog] = useState({ title: '', message: '', type: 'success' });
  const [showAuthDialog, setShowAuthDialog] = useState(false);
  ...
  useEffect(() => {
    if (isEditingName && nameInputRef.current) {
      ...
      nameInputRef.current.select(); // Optional: selects all text for easy replacement
    }
  }, [isEditingName]);
