...
//   // If a logged-in and already verified user lands here, send them away.
//   useEffect(() => {
//     if (isAuthenticated && userData?.isVerified) {
//       navigate('/dashboard', { replace: true });
//     }
...
//     try {
//       const response = await apiVerifyEmail(formData.userEmail, formData.verificationCode);
//       setFeedback({ message: (response.message || 'Email successfully verified!') + ' Redirecting...', type: 'success' });
//       await checkStatus();
...
//               <CardTitle className="text-3xl font-bold tracking-tight">Verify Your Email</CardTitle>
//               <CardDescription className="pt-2">
//                 We're sending a 6-digit code to <strong>{formData.userEmail || 'your email'}</strong>. Please enter it below.
//               </CardDescription>
//             </CardHeader>
//             <CardContent className="p-8 pt-0">
//               <form onSubmit={handleSubmit} className="space-y-6">
...
//                 <Button type="submit" className="w-full" disabled={isLoading || isResending || formData.verificationCode.length !== 6}>
//                   {isLoading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}
//                   Verify Account
//                 </Button>
//               </form>
