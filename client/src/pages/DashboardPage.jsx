...
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";
import { Alert, AlertDescription, AlertTitle } from "@/components/ui/alert";
import { Plus, FileText, AlertCircle, Sparkles, ArrowRight } from 'lucide-react';
...
// --- NEW: Dynamic "Your Next Steps" Widget ---
const ActionEngineWidget = ({ resumeCount, coverLetterCount }) => {
    ...
    return (
        <Card className="bg-gradient-to-br from-primary/10 to-transparent border-primary/20 h-full">
            ...
            <CardContent>
                <ul className="space-y-4">
                    {/* Item 1: Create Resume - Now an actionable link */}
                    <li className="flex items-center gap-3">
                        <input type="checkbox" checked={hasResume} disabled className="h-4 w-4 rounded border-gray-300 accent-primary" />
                        {hasResume ? (
                            <span className="font-medium text-muted-foreground line-through">
                                Create your first professional resume.
                            </span>
                        ) : (
                            <Link to="/templates" className="font-medium text-primary hover:underline">
                                Create your first professional resume.
                            </Link>
                        )}
                    </li>

                    {/* Item 2: Generate Cover Letter - Now an actionable link, disabled if no resume exists */}
                    <li className="flex items-center gap-3">
                        <input type="checkbox" checked={hasCoverLetter} disabled={!hasResume} className="h-4 w-4 rounded border-gray-300 accent-primary" />
                        {hasCoverLetter ? (
                             <span className={`font-medium ${!hasResume ? 'text-gray-400' : 'text-muted-foreground line-through'}`}>
                                Generate a matching cover letter.
                            </span>
                        ) : (
                             <Link to={hasResume ? "/cover-letter/generate" : "#"} className={`font-medium ${!hasResume ? 'text-gray-400 cursor-not-allowed' : 'text-primary hover:underline'}`}>
                                Generate a matching cover letter.
                            </Link>
                        )}
                    </li>
...
// --- NEW: Dynamic "Career Insights" Widget ---
const careerTips = [
    {
    ...
    },
    {
        ...
        content: `Instead of "Managed a team," try "Led a team of 5 to increase sales by 15% in Q3." Numbers grab attention.`
    },
    {
        title: "Tailor for the Job",
        content: `Always customize your resume for the specific job you're applying for. Highlight skills from the job description.`
    },
    {
        title: "Proofread Meticulously",
        content: `A single typo can get your application rejected. Use a grammar checker and have a friend review your documents.`
    }
];

const CareerInsightsWidget = () => {
    // useMemo ensures the tip doesn't change on every re-render, only on page load.
    const tip = useMemo(() => careerTips[Math.floor(Math.random() * careerTips.length)], []);
    ...
    return (
         <Card className="h-full">
            ...
            <CardContent>
                 <h4 className="font-semibold text-sm mb-2 text-primary">{tip.title}</h4>
...
const DashboardPage = () => {
    ...
    // NEW: Get user data from AuthContext
