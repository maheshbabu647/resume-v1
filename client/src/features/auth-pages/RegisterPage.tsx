    ...
    <div className={styles.page}>
      <div className={styles.leftPanel}>
        <div className={styles.leftContent}>
          ...
          <h2 className={styles.leftTitle}>Your career starts with a great resume.</h2>
          <ul className={styles.leftPoints}>
            {['Free to start — no credit card', 'AI rewriting for every section', 'ATS score on every save', 'Export as polished PDF'].map((p) => (
      ...
      <div className={styles.rightPanel}>
        <div className={styles.formCard}>
          ...
          <p className={styles.formSub}>Join 12,000+ candidates who landed their dream job</p>

          <a href={`${import.meta.env.VITE_API_URL || 'http://localhost:4000/v1'}/auth/google${referralCode ? `?ref=${referralCode}` : ''}`} className={styles.googleBtn}>
          ...
          <form className={styles.form} onSubmit={handleSubmit}>
            <Input label="Full name" placeholder="Arjun Sharma" value={name} onChange={(e) => setName(e.target.value)} required />
            ...
            <div className={styles.passWrapper}>
              <Input label="Password" type={showPass ? 'text' : 'password'} placeholder="Min. 8 characters" value={password} onChange={(e) => setPassword(e.target.value)} required />
